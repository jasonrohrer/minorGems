...
/**********************************************************************\
  Jason Rohrer, 2001-February-24:
  Added missing free() calls to susan_find_edges_thresh().
...
/*
THESE ARE NOW DEFINED IN susan.h  (Jason Rohrer, May 2000)
typedef  unsigned char uchar;
typedef  struct { float z; float w; } EDGE_ORIENTATION;
*/
...
susan_edges(in,r,mid, orient, bp,max_no,x_size,y_size)
  uchar *in, *bp, *mid;
  int   *r, max_no, x_size, y_size;
  EDGE_ORIENTATION *orient;
{
float z;
    ...
    for (j=4;j<x_size-4;j++)
    {
      if (r[i*x_size+j]>0)
      {
        ...
        if (n>600)
        {
	      ...
	      {
	       /*  center of gravity away from nucleus.  thus, use vector to c.o.g. to determine
	           edge orientation (edge perpendicular to this vector)
	           Thus, z = y / x, where z is the slope of the edge, because z is perpendicular
	           to the slope of (x,y), the vector to the c.o.g.
	        */
            ...
            if ( (m > r[(i+a)*x_size+j+b]) && (m >= r[(i-a)*x_size+j-b]) &&
              ...
              {
              mid[i*x_size+j] = 1;
              /* save orientation as well   added by Jason Rohrer */
              orient[ i*x_size+j ].z = z;
              orient[ i*x_size+j ].w = w;
              }
          }
          else
            do_symmetry=1;
        ...
        if (do_symmetry==1)
	    { 
	      /*  edge passes through center of nucleus, and ceter of gravity may be right on nucleus
	          so, use longest axis of symmetry to determine edge orientation
	          Thus, z = x / y, where z is the slope of the edge, because z is parallel
	          to the slope of (x,y), the longest axis of symmetry
	       */	
...
void susan_find_edges_thresh( uchar *in, int x_size, int y_size, uchar *binEdges, EDGE_ORIENTATION *orient, int thresh ) {
	...
	int *response;	/* edge response */
	uchar *mid;	/* (seems to be) used for edge detection and thinking */
	uchar *brightnessLUT;	/* lookup table for brighness thresholding function */
	
	int max_no_edges = 2650 ;
	
	int i;
	
	response   = (int *) malloc( x_size * y_size * sizeof(int) );
    setup_brightness_lut( &brightnessLUT, thresh, 6 );
	...
	/* compute edges */
	susan_edges(in, response, mid, orient, brightnessLUT, max_no_edges, x_size, y_size );
	/* thin edges */
	susan_thin(response, mid, x_size, y_size );
    ...
    for( i=0; i<x_size * y_size; i++ ) {
    	binEdges[i] = 0;
    	if( mid[i] < 8 ) {	/* edge here */
    		binEdges[i] = 1;
    		}
    	}
    
	/* free the memory malloc'ed earlier */
	free( response );
	free( mid );
	free( brightnessLUT ); 
	
		
	}	/* end susan_find_edges()
...
void susan_find_edges( uchar *in, int x_size, int y_size, uchar *binEdges, EDGE_ORIENTATION *orient ) {
	...
	susan_find_edges_thresh( in, x_size, y_size, binEdges, orient, 20 );
	}
